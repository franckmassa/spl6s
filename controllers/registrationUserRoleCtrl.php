...
// Instanciation de l'objet $user de la classe roles
$user = new roles();
// Appel de la méthode showRole et stockage du résultat dans la variable $showRole
...
//  On vérifie Si "idRoles" n'est pas vide et existe
    if (!empty($_POST['role'])) {
        // On vérifie si idRoles est un nombre 
        if (!is_nan($_POST['role'])) {
            $role = htmlspecialchars($_POST['role']);
            $user->idRoles = $role;
        } else {
            $formError['role'] = ERROR_IDROLES;
        }
    } else {
        $formError['role'] = FORM_EMPTY;
    }
...
// Test de regex
    ...
    /*
     * Si id existe et n'est pas vide, on stocke la valeur de l'id dans $idUsers
     * et on attribut la valeur à idUsers de l'objet $user
     */
    ...
    if (count($formError) == 0 && $resultCount == 0) {
        ...
        try {
            ...
            if ($societ->societyRegister()) {
                ...
                // on appelle la méthode UserSiteWebRegistrer de l'objet $user pour enregistrer la valeur des attributs dans la classe users
                $user->UserSiteWebRegister();
